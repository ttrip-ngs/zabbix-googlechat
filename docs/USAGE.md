...
```bash
pip install -e /path/to/zabbix-googlechat
```
...
# 依存ライブラリのインストール（Zabbixサーバーのシステムに対して）
...
source /opt/zabbix-googlechat/venv/bin/activate
pip install requests pyyaml python-dotenv

# スクリプトのshebangを仮想環境に変更
# scripts/zabbix_notify.py の1行目を以下に変更:
